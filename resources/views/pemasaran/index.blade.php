...
@section('title')Pemasaran Produk @endsection
@push('style')
...
<div id="content-container">
    ...
    <ol class="breadcrumb">
        ...
        <li class="active">Pemasaran Produk</li>
    </ol>
    <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
    <!--End breadcrumb-->
    ...
    <div id="page-content">
        <div class="panel">
            <div class="panel-heading">
                <h3 class="panel-title">Pemasaran Produk</h3>
            </div>
            <div class="panel-body">
                <a href="{{ url('pemasaran/create') }}" class="btn btn-primary"><span class="fa fa-plus"></span> Tambah Data</a>
            </div>
            <div class="panel-body">
                        ...
                        @foreach($models as $model)
                            <tr>
                                ...
                                <td>
                                  ...
                                  <a href="{{ url('pemasaran/laporan/'.$model->id) }}" type="button" class="btn btn-primary btn-block">Ajukan Laporan</a>
                                </td>
                                <td>
                                    <a href="{{ url('pemasaran/'.$model->id) }}" class="btn btn-info btn-icon icon-lg fa fa-eye"></a>
                                    <a href="{{ url('pemasaran/'.$model->id.'/edit') }}" class="btn btn-warning btn-icon icon-lg fa fa-pencil-square"></a>
...
<div id="modalAnggaran" class="modal fade" role="dialog">
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <form id="formAnggaran" method="post" action="pemasaran/anggaran">
      </form>
    </div>
  </div>
</div>
...
$('.btn.btn-danger.btn-icon.icon-lg.fa.fa-trash').on('click', function(){
    ...
    bootbox.confirm("Apakah anda yakin akan menghapus data?", function(result) {
        if (result) {
            window.location.replace(locale + '/pemasaran/delete/'+id);
        }else{
            $.niftyNoty({
                type: 'info',
                icon : 'fa fa-minus',
...
$('.btn.btn-success.btn-anggaran').on('click', function(){
    var id = $(this).attr('id');
    $.ajax({
        url: locale + '/pemasaran/get-anggaran/'+id,
