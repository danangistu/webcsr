...
@section('title')Partisipasi Hari Besar @endsection
@push('style')
...
<div id="content-container">
    ...
    <ol class="breadcrumb">
        ...
        <li class="active">Partisipasi Hari Besar</li>
    </ol>
    <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
    <!--End breadcrumb-->
    ...
    <div id="page-content">
        <div class="panel">
            <div class="panel-heading">
                <h3 class="panel-title">Partisipasi Hari Besar</h3>
            </div>
            <div class="panel-body">
                <a href="{{ url('hari-besar/create') }}" class="btn btn-primary"><span class="fa fa-plus"></span> Tambah Data</a>
            </div>
            <div class="panel-body">
                        ...
                        @foreach($models as $model)
                            <tr>
                                ...
                                <td>
                                  ...
                                  <a href="{{ url('hari-besar/laporan/'.$model->id) }}" type="button" class="btn btn-primary btn-block">Ajukan Laporan</a>
                                </td>
                                <td>
                                    <a href="{{ url('hari-besar/'.$model->id) }}" class="btn btn-info btn-icon icon-lg fa fa-eye"></a>
                                    <a href="{{ url('hari-besar/'.$model->id.'/edit') }}" class="btn btn-warning btn-icon icon-lg fa fa-pencil-square"></a>
...
$('.btn.btn-danger.btn-icon.icon-lg.fa.fa-trash').on('click', function(){
    ...
    bootbox.confirm("Apakah anda yakin akan menghapus data?", function(result) {
        if (result) {
            window.location.replace(locale + '/hari-besar/delete/'+id);
        }else{
            $.niftyNoty({
                type: 'info',
                icon : 'fa fa-minus',
...
$('.btn.btn-success.btn-anggaran').on('click', function(){
    var id = $(this).attr('id');
    $.ajax({
        url: locale + '/hari-besar/get-anggaran/'+id,
